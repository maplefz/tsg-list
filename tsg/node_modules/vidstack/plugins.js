...
const defaultIncludePattern = /\.(jsx|tsx|html|vue|svelte)/, bundleModuleId = "vidstack/bundle", mediaElementPrefix = "media-", defaultThemeClassPrefix = "vds-", elementsFilePath = path$1.resolve(__dirname, "./elements.json"), elementsJSON = fs.readFileSync(elementsFilePath, "utf8"), elementsManifest = JSON.parse(elementsJSON);
const defaultStyles = {
...
const unplugin = createUnplugin((options = {}) => {
  let include = options.include ?? defaultIncludePattern, filter = createFilter(include, options.exclude), data = {
    elementsGraph: /* @__PURE__ */ new Map(),
    stylesGraph: /* @__PURE__ */ new Map()
  }, defaultAudioLayout = false, defaultVideoLayout = false, plyrLayout = false, chunkToFile = /* @__PURE__ */ new Map(), moduleToChunk = /* @__PURE__ */ new Map(), viteServer = null;
  function hasDefaultLayout() {
    return defaultAudioLayout || defaultVideoLayout;
  }
  function processModule(id, code) {
    const { elementsGraph, stylesGraph } = data, { elements, styles } = parse(code);
    if (elements.size) {
  ...
  return [
    {
      ...
      load(id) {
        ...
        if (chunkToFile.has(id)) {
          ...
          const moduleId = chunkToFile.get(id), elements = data.elementsGraph.get(moduleId), styles = data.stylesGraph.get(moduleId);
      ...
      vite: {
        ...
        async handleHotUpdate({ file, read }) {
          ...
          const oldElements = data.elementsGraph.get(file), oldStyles = data.stylesGraph.get(file);
          const { elements: newElements, styles: newStyles } = processModule(file, await read());
          const hasChangedLayouts = diff(oldElements, newElements, "media-audio-layout") || diff(oldElements, newElements, "media-video-layout") || diff(oldElements, newElements, "media-plyr-layout");
    ...
    },
    {
      ...
      transform(code, id) {
        const { elementsGraph, stylesGraph } = data, elements = elementsGraph.get(id), styles = stylesGraph.get(id);
...
const PARSE_MODE_NONE = 0, PARSE_MODE_ELEMENT = 1, PARSE_MODE_STYLE = 2;
function parse(code) {
  let mode = PARSE_MODE_NONE, char = "", buffer = "", elements = /* @__PURE__ */ new Set(), styles = /* @__PURE__ */ new Set();
  for (let i = 0; i < code.length; i++) {
    ...
    switch (mode) {
      case PARSE_MODE_NONE:
        ...
        } else if (char === "v" && code[i + 1] === "d" && code[i + 2] === "s" && code[i + 3] === "-") {
          buffer = "v";
      ...
      case PARSE_MODE_STYLE:
        ...
        } else if (char === " " || char === "\n" || char === "\r" || buffer.length > defaultThemeClassPrefix.length) {
          mode = PARSE_MODE_NONE;
        } else {
          buffer += char;
        }
...
function generateBundleImports({
  ...
  const styles = [], imports = [], hasDefaultLayout = defaultAudioLayout || defaultVideoLayout;
  if (!hasDefaultLayout) {
...
function generateElementImports(elements = /* @__PURE__ */ new Set(), styles = /* @__PURE__ */ new Set()) {
  const imports = [], elementClasses = /* @__PURE__ */ new Set();
...
function isSetsEqual(a, b) {
  return !a?.size && !b?.size || !!a && !!b && a.size === b.size && [...a].every((value) => b.has(value));
}
const vite = (options) => unplugin.vite(options);
const rollup = (options) => unplugin.rollup(options);
const webpack = (options) => unplugin.webpack(options);
const rspack = (options) => unplugin.rspack(options);
const esbuild = (options) => unplugin.esbuild(options);

export { esbuild, rollup, rspack, unplugin, vite, webpack };
