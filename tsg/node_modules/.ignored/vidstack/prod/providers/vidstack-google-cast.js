import { listenEvent, effect, untrack, createScope, keysOf, onDispose, DOMEvent, peek } from '../chunks/vidstack-C6myozhB.js';
...
import { getCastSessionMedia, getCastContext, getCastSession, hasActiveCastSession, listenCastContextEvent, getCastErrorMessage } from '../chunks/vidstack-D2Jcqk64.js';
...
class GoogleCastTracksManager {
  ...
  pj() {
    ...
    if (!this.od.isMediaLoaded) return;
    const remoteTextTracks = this.xc(REMOTE_TRACK_TEXT_TYPE);
...
class GoogleCastProvider {
  ...
  async play() {
    if (!this.f.isPaused && !this.Be) return;
    if (this.Be) {
  ...
  async loadSource(src) {
    if (this.Pa?.src !== src) this.Pa = null;
  ...
  Bj() {
    ...
    if (currentTime === this.ua) return;
    this.c("time-change", currentTime);
    if (currentTime > this.ga) {
      this.ga = currentTime;
      this.nb();
    }
  ...
  de(event) {
    if (!this.f.isMediaLoaded || this.Pa) return;
  ...
  nb(event) {
    const detail = {
      seekable: this.Fg(),
      buffered: new TimeRange(0, this.ga)
